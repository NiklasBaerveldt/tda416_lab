...
public class DLList {
	...
	*/
	public void addLast(Point p) throws NullPointerException{
        ...
        if(head == null)
        {
            head = new Node(p,size++);
            tail = head;
        }
        else {
            tail.next = new Node(p, size++);
            tail.next.prev = tail;
            tail = tail.next;
        }
		// TODO
	...
	*/
	...
	public void reduceListToKElements(int k) {
		...
		while(q.size() > k)
		{
			...
			if(nextNode != tail) {
                nextNode.imp = importanceOfP(nextNode.prev.p, nextNode.p, nextNode.next.p);
            }
            // and rem.prev, neighbour to the left
			if(prevNode != head) {
			    prevNode.imp = importanceOfP(prevNode.prev.p,prevNode.p,prevNode.next.p);
            }
			q.remove(nextNode);
			q.remove(prevNode);
            q.offer(prevNode);
